...
Hard-won rules for making the whole page one continuous generated film (Higgsfield
Seedance is the reference engine; any start-image-capable image-to-video engine follows
the same contract). These are a floor, not a ceiling — break them knowingly, never by
accident.
...
## 9. Governance
Design taste and design code are done by the director model only — the frontier model
running the skill, whichever family that is. Mechanical steps (ffmpeg, SSIM, puppeteer,
vercel) are pure code — no model. Quote credits before spending; show the receipt after.
One continuous shot, one world per brand.

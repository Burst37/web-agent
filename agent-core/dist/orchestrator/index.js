...
function buildFieldChecklist(schema) {
  if (!schema) return "";
...
function buildColumnsBlock(columns) {
  if (!columns?.length) return "";
  return `Required columns (each is a data point to collect):
${columns.map((c) => `- ${c}`).join("\n")}`;
}
function buildFormatInstructions(schema, columns) {
  if (schema) {
...
async function createOrchestrator(options) {
  ...
  const subAgentModel = config.subAgentModel ? await resolveModel(config.subAgentModel, apiKeys) : model;
...
6. Call formatOutput with the collected data. The task is not done until formatOutput is called.`,
    ...
    urlHint: config.urls?.length ? `<user_urls>
Start with these URLs: ${config.urls.join(", ")}
</user_urls>` : "",
    uploadHint: uploadDescriptions.length > 0 ? `<uploaded_files>
